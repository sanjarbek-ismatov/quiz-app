...
// Output: JSON object with questions: [...]
  ...
  return (
    <div className="space-y-8 fade-in max-w-2xl">
      <div>
        ...
        <p className="text-gray-600 mt-2">Import your questions in bulk and generate a subject JSON file</p>
      </div>

      <div className="card p-8 space-y-6">
        <div>
          <label htmlFor="subject-select" className="block text-sm font-semibold text-gray-900 mb-3">
            Target Subject ID (for filename)
          </label>
          <select 
            id="subject-select"
            value={subject} 
...
...`}
        ...
        <div className="flex gap-3 pt-4">
          <button onClick={handlePreview} disabled={!raw.trim()} className="btn-secondary flex-1 gap-2">Preview</button>
          <button onClick={handleImport} disabled={!raw.trim()} className="btn-primary flex-1 gap-2">
            <FiDownload className="text-lg" aria-hidden="true" />
            Generate & Download
          </button>
          <button onClick={() => { setRaw(''); setParseResult(null); }} className="btn-secondary gap-2" title="Clear all text">
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-6">
        ...
        <ol className="space-y-2 text-sm text-gray-700 list-decimal list-inside">
          <li>Add the subject entry to <code className="bg-amber-100 px-2 py-1 rounded font-mono text-xs">src/config/subjects.ts</code></li>
          <li>Use this tool to generate the JSON file</li>
          <li>Place the downloaded file in <code className="bg-amber-100 px-2 py-1 rounded font-mono text-xs">src/data/subjects/</code></li>
          <li>That's it! Everything will update automatically.</li>
        </ol>
      </div>
    </div>
  )
